...
export default function App() {
  ...
  async function newChat() {
    ...
    let newChatId;
    if (chats.length === 0) newChatId = 0;
    else newChatId = chats[chats.length - 1].id + 1;
    chats.push({
      id: newChatId,
      name: "New Chat",
      date: generateDate(),
  ...
  async function handleRenameChat(id, name) {
    ...
    const updatedChats = await getChats();
    setChats(updatedChats);
    if (currentChatId === id) setCurrentChatName(name);
    hideRename();
  }
  ...
  function generateTimelineLabel(chat) {
    if (chat.length > 0) {
      ...
      if (
        ...
        today.getMonth() + 1 === comparedDate.getMonth() &&
        today.getFullYear() == comparedDate.getFullYear()
      ) {
        return i18n.t("today");
      } else if (
        today.getDate() === comparedDate.getDate() + 1 &&
        today.getMonth() + 1 === comparedDate.getMonth() &&
        today.getFullYear() == comparedDate.getFullYear()
      ) {
  ...
  function sortChats(chats) {
    ...
    if (todayChats.length > 0) todayChats = sortChatsFromDates(todayChats);

    if (yesterdayChats.length > 0)
      yesterdayChats = sortChatsFromDates(yesterdayChats);

    if (pastChats.length > 0) pastChats = sortChatsFromDates(pastChats);
          ...
          return (
            <View style={styles.drawer}>
              <View style={styles.chats}>
                ...
                {sortedChats.map((chatList, index) => {
                  if (chatList.length === 0) return null;

                  return (
                    <View key={index}>
                      <Text style={styles.chatTimeStamp}>
                        {generateTimelineLabel(chatList)}
                      </Text>
                      <FlatList
